ALTER TABLE users
  ADD COLUMN public_uid VARCHAR(8),
  ADD COLUMN college_id BIGINT,
  ADD COLUMN major_id BIGINT,
  ADD COLUMN operator_scope VARCHAR(30) NOT NULL DEFAULT 'student',
  ADD COLUMN level INT NOT NULL DEFAULT 1,
  ADD COLUMN xp INT NOT NULL DEFAULT 0,
  ADD COLUMN level_title VARCHAR(80) NOT NULL DEFAULT '萌新探路员';

UPDATE users SET public_uid = LPAD(CAST(10000000 + id AS CHAR), 8, '0') WHERE public_uid IS NULL;
ALTER TABLE users MODIFY public_uid VARCHAR(8) NOT NULL;
ALTER TABLE users ADD UNIQUE KEY uk_users_public_uid (public_uid);
...
CREATE TABLE colleges (
  ...
  UNIQUE KEY uk_college_school_code (school_id, code),
...
CREATE TABLE majors (
  ...
  created_at DATETIME NOT NULL,
  updated_at DATETIME NOT NULL,
  UNIQUE KEY uk_major_college_code (college_id, code),
  CONSTRAINT fk_majors_school FOREIGN KEY (school_id) REFERENCES schools(id),
...
CREATE TABLE posts (
  ...
  created_at DATETIME NOT NULL,
  updated_at DATETIME NOT NULL,
  INDEX idx_posts_school_board (school_id, board, status, published_at),
  INDEX idx_posts_hot (status, like_count, comment_count),
...
CREATE TABLE post_likes (
  ...
  post_id BIGINT NOT NULL,
  created_at DATETIME NOT NULL,
  UNIQUE KEY uk_post_like_user (user_id, post_id),
...
CREATE TABLE post_favorites (
  ...
  post_id BIGINT NOT NULL,
  created_at DATETIME NOT NULL,
  UNIQUE KEY uk_post_favorite_user (user_id, post_id),
...
CREATE TABLE post_daily_stats (
  ...
  comment_count INT NOT NULL DEFAULT 0,
  share_count INT NOT NULL DEFAULT 0,
  UNIQUE KEY uk_post_daily_stat (post_id, stat_date),
...
CREATE TABLE media_objects (
  ...
  purpose VARCHAR(40),
  created_at DATETIME NOT NULL,
  CONSTRAINT fk_media_objects_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

ALTER TABLE comments MODIFY notice_id BIGINT NULL;
